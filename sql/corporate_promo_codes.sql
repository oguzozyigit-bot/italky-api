...
create table if not exists public.promo_codes (
  ...
  note text null,
  constraint promo_codes_status_check check (status in ('active','activated','expired','cancelled')),
  constraint promo_codes_duration_check check (duration_months in (1,3,6,12) and duration_days > 0),
...
create index if not exists promo_codes_company_idx on public.promo_codes(company_name);
...
create index if not exists promo_codes_activated_at_idx on public.promo_codes(activated_at);
create index if not exists promo_codes_membership_ends_idx on public.promo_codes(membership_ends_at);
