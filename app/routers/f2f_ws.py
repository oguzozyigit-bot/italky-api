...
# ============================================================
# STAGING: isolated low-latency F2F test lane (no floor/games)
# ============================================================

STAGING_ROOMS: Dict[str, Dict[str, Any]] = {}


def staging_get_room(room_id: str) -> Optional[Dict[str, Any]]:
    rid = norm_room_id(room_id)
    r = STAGING_ROOMS.get(rid)
    if not r:
        return None
    if room_expired(r):
        try:
            del STAGING_ROOMS[rid]
        except Exception:
            pass
        return None
    return r
...
def staging_create_room(room_id: str) -> Dict[str, Any]:
    rid = norm_room_id(room_id)
    r = {
        "created_at": now(),
        "updated_at": now(),
...
def canonical_lang(value: str, fallback: str = "tr") -> str:
    v = clean_lang(value, fallback)
    return v.split("-")[0]


def google_translate_staging(text: str, source: str, target: str) -> str:
    import requests

    src = canonical_lang(source, "auto")
    tgt = canonical_lang(target, "en")
    if not text.strip():
        return ""
    if src == tgt:
        return text.strip()
    ...
    if not data or not isinstance(data[0], list):
        return text.strip()
    parts = [str(chunk[0] or "") for chunk in data[0] if chunk and chunk[0]]
    out = "".join(parts).strip()
    return out or text.strip()


async def staging_broadcast(room: Dict[str, Any], msg: Dict[str, Any], exclude: Optional[WebSocket] = None) -> None:
    dead: List[WebSocket] = []
    for c in list(room["clients"]):
        if exclude is not None and c is exclude:
            continue
...
@router.websocket("/f2f/staging/ws/{room_id}")
async def f2f_staging_ws(ws: WebSocket, room_id: str):
    rid = norm_room_id(room_id)
    await ws.accept()
    ...
    try:
        while True:
            ...
            if mtype == "ping":
                client_ts = int(msg.get("client_ts") or 0)
                await ws_send(ws, {
                    "type": "pong",
                    "client_ts": client_ts,
                    "server_ts": server_ts,
            ...
            if mtype == "create":
                if joined_room is not None:
                    await ws_send(ws, {"type": "error", "message": "ALREADY_JOINED"})
                    continue
                r = staging_get_room(rid) or staging_create_room(rid)
                joined_room = r
                my_meta = {
                    "peer_id": clean_user_id(msg.get("peer_id") or msg.get("from") or f"p_{server_ts}"),
                    "name": clean_name(msg.get("name"), "Host"),
                    "me_lang": clean_lang(msg.get("me_lang"), "tr"),
                    "target_lang": clean_lang(msg.get("target_lang"), "en"),
                    "role": "host",
                }
                r["clients"].add(ws)
                r["meta"][ws] = my_meta
                r["updated_at"] = now()
                await ws_send(ws, {"type": "room_created", "room": rid, "self": my_meta, "staging": True})
                await staging_broadcast(r, {"type": "presence", "count": len(r["clients"]), "staging": True})
                continue
            ...
            if mtype == "join":
                ...
                my_meta = {
                    "peer_id": clean_user_id(msg.get("peer_id") or msg.get("from") or f"p_{server_ts}"),
                    "name": clean_name(msg.get("name"), "Guest"),
                    "me_lang": clean_lang(msg.get("me_lang"), "en"),
                    "target_lang": clean_lang(msg.get("target_lang"), "tr"),
                    "role": "guest",
                }
                r["clients"].add(ws)
                r["meta"][ws] = my_meta
                r["updated_at"] = now()
                await ws_send(ws, {"type": "room_joined", "room": rid, "self": my_meta, "staging": True})
                await staging_broadcast(r, {
                    "type": "peer_joined",
                    "peer": my_meta,
                    "count": len(r["clients"]),
                }, exclude=ws)
            ...
            if mtype == "audio_chunk":
                ...
                payload = {
                    "type": "audio_chunk",
                    "from": my_meta.get("peer_id") or "",
                    "name": my_meta.get("name") or "",
                    "seq": int(msg.get("seq") or 0),
                    "mime": str(msg.get("mime") or "audio/webm"),
                    "chunk_b64": str(msg.get("chunk_b64") or ""),
                    "client_ts": int(msg.get("client_ts") or 0),
                    "server_ts": server_ts,
                }
                if payload["chunk_b64"]:
                    await staging_broadcast(joined_room, payload, exclude=ws)
                continue
            ...
            if mtype == "utterance":
                ...
                client_ts = int(msg.get("client_ts") or server_ts)
                source_lang = clean_lang(msg.get("source_lang"), my_meta.get("me_lang") or "tr")
                target_lang = clean_lang(msg.get("target_lang"), my_meta.get("target_lang") or "en")
                ...
                await staging_broadcast(joined_room, relay, exclude=ws)
                ...
                try:
                    ...
                    translated = await asyncio.to_thread(
                        google_translate_staging,
                        text,
                        source_lang,
                        target_lang,
                    )
                    translate_ms = int((now() - t0) * 1000)
                ...
                out = {
                    ...
                    "total_ms": max(0, server_ts - client_ts) if client_ts else translate_ms,
                    "staging": True,
                }
                await staging_broadcast(joined_room, out)
                await ws_send(ws, out)
                joined_room["updated_at"] = now()
                continue
        ...
        if joined_room is not None:
            try:
                leaving = joined_room["meta"].get(ws, {}) or {}
                joined_room["clients"].discard(ws)
                joined_room["meta"].pop(ws, None)
                joined_room["updated_at"] = now()
                await staging_broadcast(joined_room, {
                    "type": "peer_left",
                    "peer": leaving,
                    "count": len(joined_room["clients"]),
                })
